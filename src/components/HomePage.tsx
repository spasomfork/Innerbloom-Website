...
export const HomePage: React.FC<HomePageProps> = ({ onNavigate, onOpenBooking }) => {
  ...
  return (
    <div id="home-page" className="w-full">
      ...
      {/* 2. ABOUT US BANNER SECTION */}
      <section
        id="home-about-banner"
        onClick={() => {
          onNavigate('about');
          window.scrollTo({ top: 0, behavior: 'smooth' });
        }}
        className="relative h-64 sm:h-80 md:h-96 bg-cover bg-center flex items-center justify-center cursor-pointer group overflow-hidden"
        style={{
          backgroundImage: `url('${IMAGES.flowerBanner}')`,
        }}
      >
        <div className="absolute inset-0 bg-black/30 group-hover:bg-black/20 transition-all duration-500" />
        <div className="relative z-10 text-center text-white px-4 transform group-hover:scale-105 transition-transform duration-500">
          <h2 className="text-4xl sm:text-5xl md:text-6xl font-bold tracking-tight drop-shadow-md">
            About Us
          </h2>
          <span className="inline-flex items-center gap-1.5 mt-2 text-xs font-semibold text-emerald-200 tracking-widest uppercase opacity-0 group-hover:opacity-100 transition-opacity">
            Learn More About Our Practice <ArrowRight className="w-3.5 h-3.5" />
          </span>
        </div>
      </section>
              ...
              <div className="lg:col-span-5 space-y-6 self-center">
                ...
                <div className="space-y-4 text-sm text-gray-200">
                  <div className="flex items-start gap-3">
                    ...
                    <p className="leading-snug">{SITE_INFO.addressAlt}</p>
                  </div>

                  <div className="flex items-center gap-3">
                    <Phone className="w-4 h-4 text-emerald-400 shrink-0" />
                    <a
                      href={`tel:${SITE_INFO.phoneContact.replace(/\s+/g, '')}`}
                      className="hover:text-emerald-300 transition-colors"
                    >
                      {SITE_INFO.phoneContact}
                    </a>
                  </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <a
                      href={`mailto:${SITE_INFO.emailContact}`}
                      className="hover:text-emerald-300 transition-colors"
                    >
                      {SITE_INFO.emailContact}
                    </a>
                  </div>
                </div>
